 ...
 * @since v0.1.0
 */
class AFOrganizationListing extends AFConcreteStructureListing {
    /**
     ...
     * initialized.
     */
    static createForClient(client, skipInitializationRefresh = false) {
        return __awaiter(this, void 0, void 0, function* () {
            let organizationListing = new AFOrganizationListing(client);
            if (!skipInitializationRefresh) {
                let organizationResponses = yield AFAPIOrganizationAccessor.getBasesForSession(organizationListing.getSession().getToken());
                for (let organizationResponse of organizationResponses) {
                    ...
                    let organization = yield AFOrganization.createWithAPIResponse(organizationListing.getClient(), organizationResponse);
    ...
    refresh() {
        return __awaiter(this, void 0, void 0, function* () {
            let readOnlyOrganizations = yield AFAPIOrganizationAccessor.getBasesForSession(this.getSession().getToken());
            let serverOrganizations = new Map();
            let localOrganizationIDs = new TSArrayList(...this.getAllIDs());
            for (let serverOrganization of readOnlyOrganizations)
                serverOrganizations.set(serverOrganization.id, serverOrganization);
            for (let id of localOrganizationIDs.iterator())
                if (!serverOrganizations.has(id))
                    this.removeMapping(id);
            for (let id of serverOrganizations.keys()) {
                if (!localOrganizationIDs.contains(id)) {
                    let organization = yield AFOrganization.createWithAPIResponse(this.getClient(), serverOrganizations.get(id));
    ...
    getAllSignedInOrganizations() {
        let allOrgs = this.getAll();
        return allOrgs.filter((organization) => organization.isSignedIn());
    }
    getSession() {
        return this.session;
    }
...
//# sourceMappingURL=af-organization-listing.js.map